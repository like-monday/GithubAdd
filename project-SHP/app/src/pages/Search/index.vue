                ...
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a
                    >综合<span
                      ...
                      class="iconfont"
                      :class="{
                        'icon-angle-double-up': isAsc,
                        'icon-angle-double-down': isDesc,
                      }"
                    ></span
                  ></a>
                </li>
...
export default {
  ...
  methods: {
    //向服务器发请求获取search模块数据（根据参数不同返回不同得数据进行展示）
    //把这次请求封装为一个函数：当你需要在调用得时候调用即可
    getData() {
    ...
    removeCategoryName() {
      ...
      //但是你吧相应的字段变为undefined，当前这个字段不会带给服务器
      ...
      //严谨：本意真删除query，如果路径中出现params不应该删除，路由跳转的时候应该带着
  ...
  watch: {
    //监听路由的信息是否发生变化，如果生变化，再次发起请求
    $route(newValue, oldValue) {
      //再次发请求之前整理带给服务器参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      //再次发起ajax请求
      this.getData();
      //每一次请求完毕，应把相应的1、2、3级分类的id置空，让他接受下一次的相应1，2，3id
      //分类名字与关键字不用清理：因为每一次路由发生变化的时间，都会给他赋予新的数据
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    },
